...
> :tip:
> 1. Каждый создаваемый и запускаемый поток сам может создавать и запускать другие потоки;
     > Пример:
>
> ```java
...
> ```
>
> 2. Способ создания потоков посредством создания реализации интерфейса `Runnable` и передачи её в поток является более
