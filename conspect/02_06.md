...
private static final int COUNTER_ADD_VALUE = 500;
// Шаблон сообщения исключения
private static final String EXCEPTION_MESSAGE_TEMPLATE = "Exception message: %s in thread %s\n\r";
...
private static class Task implements Runnable {
    @Override
    public void run() {
        for (int i = 0; i < COUNTER_ADD_VALUE; i++) {
            counter++;
            try {
...
> [!NOTE]
> 
> В данном примере потоки запускаются планировщиком потоков операционной системы и приостанавливают свою работу на
> основании установленного в нём 
> [квантования времени выполнения](/conspect/02_06.md/#26-многопоточность-multithreading).
> Также потоки изменяют общий ресурс в отведённый для их выполнения 
> [квант](/conspect/02_06.md/#26-многопоточность-multithreading) без проведения предварительной проверки факта
> изменения его значения в период приостановки.
