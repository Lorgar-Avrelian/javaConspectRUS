...
Ещё одно отличие `ReentrantLock` от блоков $\textrm{\color{orange}synchronized}$ - это свойство _fair_. Блокировка с
этим свойством обеспечивает «справедливость» очереди: пришедший раньше поток захватывает объект раньше. Блок
$\textrm{\color{orange}synchronized}$ не даёт никаких гарантий порядка.
