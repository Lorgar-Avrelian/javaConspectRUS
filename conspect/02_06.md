...
Статус демона виртуального потока всегда равен $\textrm{\color{orange}true}$ и не может быть изменен с помощью метода
...
> [!NOTE]
> 
> В случае наличия в потоке-демоне блока $\textrm{\color{orange}finally}$ при завершении его работы _JVM_ содержащийся в блоке код не выполнит.  
