...
Он позволяет потоку ожидать выполнения определённого условия, а другим потокам - сигнализировать о его наступлении,
обеспечивая более гибкий и мощный контроль над потоковой синхронизацией в сочетании с интерфейсом `Lock`.

Интерфейс `Condition` используется для управления ожиданием и уведомлением потоков в многопоточной среде, заменяя
стандартные методы `wait()`, `notify()` и `notifyAll()` объектов-мониторов. Он позволяет потоку приостановить выполнение
(`await()`) до тех пор, пока другой поток не уведомит его о выполнении определённого условия состояния (`signal()` или
`signalAll()`).  
Реализация интерфейса `Condition` всегда связана с реализацией интерфейса `Lock`, которая используется для защиты
...
- замена методов монитора - вместо использования $\textrm{\color{orange}synchronized}$ блоков и вызова `wait()`,
  `notify()`, `notifyAll()` на объектах-мьютексах, `Condition` позволяет использовать явные блокировки `Lock` и объекты
  `Condition` для того же функционала.
