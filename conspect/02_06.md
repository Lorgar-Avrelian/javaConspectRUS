...
Метод `wait()` класса `Object` освобождает данный монитор (объект синхронизации) до тех пор, пока для монитора не будут 
вызваны методы `notify()` или `notifyAll()`, или на указанный промежуток времени.  
Синтаксис метода выглядит следующим образом:
...
> _lock_, посредством которого производится блокировка выполнения ими $\textrm{\color{orange}synchronized}$ блоков. При
