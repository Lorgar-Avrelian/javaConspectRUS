...
Метод `setUncaughtExceptionHandler()` класса `Thread` позволяет создать реализацию обработчик необрабатываемых
исключений (интерфейса `Thread.UncaughtExceptionHandler`) и установить его для обработки необрабатываемых исключений
данного потока.  
Синтаксис метода выглядит следующим образом:

$\textrm{\color{white}thread.setUncaughtExceptionHandler(\color{purple}[обработчик необрабатываемых исключений]\color{white})}$
...
public static void main(String[] args) {
    ...
    Thread thread = new Thread(task);
    // Установка обработчика необрабатываемых исключений для потока
...
Метод `setDefaultUncaughtExceptionHandler()` класса `Thread` позволяет создать реализацию обработчик необрабатываемых
исключений (интерфейса `Thread.UncaughtExceptionHandler`) и установить его по умолчанию для обработки необрабатываемых
...
public static void main(String[] args) {
    ...
    Thread thread0 = new Thread(task0);
    Thread thread1 = new Thread(task1);
    // Установка обработчика необрабатываемых исключений для всех потоков
    Thread.UncaughtExceptionHandler exceptionHandler = new ExceptionHandler();
