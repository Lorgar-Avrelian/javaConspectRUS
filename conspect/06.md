...
Как показала практика, данные настройки не позволяют _Swagger UI_ корректно сохранять _CSRF_-токен в запросах, из-за
...
### 6.2.1 Совместимость со Spring Security
...
[**Mapper (маппер)**](/conspect/definitions.md/#m) - это интерфейс или класс, содержащий методы для преобразования одних
объектов в другие.
...
Для подключения библиотеки _Lombok_ необходимо добавить зависимость в файл:
...
После подключения библиотеки _Lombok_ к проекту можно использовать аннотации, предоставляемые ею, для замещения
шаблонного кода.
...
Когда после аннотировано `@Getter` и/или `@Setter`, _Lombok_ автоматически сгенерирует заданный по умолчанию геттер
...
С этой аннотацией _Lombok_ по умолчанию делает все поля `private final`. Кроме того, сеттеры не генерируются, а класс
как таковой помечается `final`. Таким образом, от этого класса нельзя наследовать.  
Как и в случае с `@Data`, создаются реализации `toString()`, `equals()` и `hashCode()`.
...
Аннотация `@Cleanup` позволяет гарантировать, что заданный ресурс будет автоматически очищаться перед тем, как покинет
актуальную область видимости.  
По умолчанию предполагается, что метод очистки аннотированного ресурса будет `close()`, но можно самостоятельно указать
имя того метода, который будет вызываться вместо него.  
Эта аннотация активно использует команды _try-with-resources_.
...
#### 6.7.2.9 @Synchronized

> [[_оглавление_]](../README.md/#67-преобразование-кода)

Аннотация `@Synchronized` действует примерно так же, как и ключевое слово `synchronized`, но синхронизирует на других
объектах.

Ключевое слово синхронизирует на основе `this`, а эта аннотация - на особом приватном поле под названием `$lock`. Если
это поле не существует, то _Lombok_ создаст его. Такое поведение задано по умолчанию, но можно и самостоятельно указать
те объекты, на которых будет происходить синхронизация.  
При работе с методами `static` аннотация будет синхронизировать на статическом поле `$LOCK`.  
Эта аннотация, как и ключевое слово `synchronized`, может использоваться только со статическими методами и методами
экземпляра.
...
#### 6.7.2.10 @SneakyThrows

> [[_оглавление_]](../README.md/#67-преобразование-кода)

Аннотация `@SneakyThrows` позволяет выбрасывать проверяемые исключения, не объявляя их явно в условии `throws` метода,
как принято делать.  
Эта аннотация позволяет полностью избавиться от блоков `try-catch`, поскольку обрабатывает все проверяемые исключения. В
противном случае она запутала бы компилятор. Фактически, на уровне файлов классов _JVM_, все исключения могут
выбрасываться безотносительно условия `throws`, присутствующего в методах, поэтому-то данный механизм и работает.

#### 6.7.2.11 @Builder

> [[_оглавление_]](../README.md/#67-преобразование-кода)

Для класса, аннотированного `@Builder`, _Lombok_ сгенерирует объект-строитель (_builder_).  
Аннотируя класс при помощи `@Builder`, _Lombok_ выдаёт класс, реализующий вышеупомянутый паттерн «строитель».

#### 6.7.2.12 @Log

> [[_оглавление_]](../README.md/#67-преобразование-кода)

Большинство средств логирования требуют закладывать экземпляр логгера для каждого класса, в котором нужно вести лог.
Разумеется, при этом возникает шаблонный код. _Lombok_, для аннотированного класса при помощи `@Log`, автоматически
добавляет статическое финальное поле `log`, инициализируемое библиотекой логирования как требуемое.

_Lombok_ предоставляет разработчикам по одной аннотации на каждый из наиболее популярных фреймворков
логирования: `@Log4j`, `@Log4j2`, `@Slf4j`, `@Log` (для использования _Logback_).