...
[**Инкапсуляция**](/conspect/definitions.md/#и) - это принцип ООП, согласно которого мы не даём прямой доступ к
...
Для реализации принципа наследования в _Java_ используется ключевое слово <font color="orange">$\textrm{\color{orange}extends}$</font>.
...
Если влезть в реализацию этого механизма, то _Java_ неявно подкладывает объект класса-родителя в ваш объект и 
присваивает его переменной $\textrm{\color{orange}super}$ (по аналогии с $\textrm{\color{orange}this}$, которая 
ссылается на ваш текущий объект).  
