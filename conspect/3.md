...
| `@RequestParam`  | обозначается аргумент метода, получаемый из _HTTP_-запроса | `name`<br/>`required` | указывает название параметра в запросе<br/>обязательность параметра (по умолчанию `true`)                                              |                                                                                                      |
